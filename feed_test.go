...
func TestAtomFeedParsing(t *testing.T) {
	...
	if author := atom.Author.Name; author != "EventStore" {
		t.Errorf("Unexpected author name: %v", author)
	}

	if l := len(atom.Entries); l != 20 {
		t.Errorf("Unexpected entries count: %v", l)
	}

	entryLink := atom.Entries[0].Link
	if linkCount := len(entryLink); linkCount != 2 {
		t.Errorf("Unexpected link count for first entry: %v", linkCount)
	}
}
